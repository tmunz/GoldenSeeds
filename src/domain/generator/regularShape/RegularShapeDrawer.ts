...
export function draw(config: RegularShapeConfig, grid: number[][]) {
  ...
  return grid.map((position: number[], i: number) => {
    const n = i + 1;
...
function getItemProps(config: RegularShapeConfig, n: number, items: number, size: number): ItemProps {
...
function drawRegularShape(center: number[], itemProps: ItemProps, style: string): string {
  ...
  return `<path ${style} d="${pts.reduce((s, pt, j) => {
    if (i === 0 && j === 0) {
      ...
      return s + (isSamePosition(pts[(j - 1) % pts.length], pt) ? '' : ' M ' + p(pt));
    } else {
...
function drawArc(center: number[], n: number, size: number, startAngle: number, angle: number, style: string): string {
  if (angle <= -360 || 360 <= angle) {
    return `<circle ${style} cx="${center[0]}" cy="${center[1]}" r="${size / 2}" />`;
  } else {
    const r = size / 2;
    ...
    return `<path ${style} d="${
      `M ${pointAsSvg(startPt)} A ${r},${r} 0 ${angle % 360 < 180 ? 0 : 1}  1 ${pointAsSvg(endPt)}`
      }" />`;
  }
}
