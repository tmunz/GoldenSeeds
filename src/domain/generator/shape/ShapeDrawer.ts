...
export interface ShapeConfig {
  ...
  edges: number;
  offset: (n: number, items: number, itemSize: number, i: number) => number;

  smoothness: (n: number, items: number, itemSize: number) => number;
  noise: number;
  ...
  return grid.reduce(
    (agg, position: Point, i: number) => {
      ...
      if (config.edges <= 1 && config.coordinateType === 'polar') {
        ...
        svg = drawArc(position, itemSize, startAngle, angle, elementStyle);
        boundingBox = {
          min: [
        ...
        const points = [
          () =>
            generateShape(config.edges, (j: number) =>
              config.offset(n, i, itemSize, j),
            ),
          (pnts: Point[]) =>
          ...
          (pnts: Point[]) => noise(pnts, n, config.noise, config.seed),
          (pnts: Point[]) =>
            config.coordinateType === 'polar' ? toPolar(pnts) : pnts,
          (pnts: Point[]) => smooth(pnts, itemProps.smoothness),
          (pnts: Point[]) =>
            transform(pnts, position, itemProps.size, itemProps.angle),
...
function probabilityDistribution(
  ...
  return pnts.map((pnt, i) => {
    ...
    const dy =
      pdf * scale -
      noise * pdf * random(0, scale, seed + (n * pnts.length + i) * 1956);
...
function noise(pnts: Point[], n: number, noise: number, seed: number): Point[] {
  return pnts.map((pnt, i) => {
    const dy = random(
      noise * -0.01,
      noise * 0.01,
      seed + (n * pnts.length + (i % (pnts.length - 1))),
    );
    return [pnt[Point.X], pnt[Point.Y] + dy];
  });
}
...
function smooth(pnts: Point[], smoothness: number): Point[] {
  return pnts.reduce((smoothedPnts, pnt, i) => {
    ...
    const correctionFactor =
      ((2 / 3) * Math.tan(Math.PI / 2 / n)) / Math.sin(Math.PI / n);
    ...
    const dPrev =
      smoothness * correctionFactor * PointUtils.distance(pnt, pntPrev);
    const dNext =
      smoothness * correctionFactor * PointUtils.distance(pnt, pntNext);

    return [
      ...smoothedPnts,
      [
        pnt[Point.X] + dPrev * Math.cos(rads + Math.PI),
        pnt[Point.Y] + dPrev * Math.sin(rads + Math.PI),
      ],
      pnt,
      [
        pnt[Point.X] + dNext * Math.cos(rads),
        pnt[Point.Y] + dNext * Math.sin(rads),
      ],
    ];
  }, [] as Point[]);
}
...
function transform(
  ...
  const absolutePosition = (orig: Point, pt: Point) => [
    orig[Point.X] + pt[Point.X],
    orig[Point.Y] - pt[Point.Y],
  ];
...
function getItemProps(
  ...
  return { size, edges, angle, smoothness, cutRatio0, cutRatio1 };
}
