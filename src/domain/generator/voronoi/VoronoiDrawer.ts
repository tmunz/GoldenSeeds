...
export function draw(config: VoronoiConfig, grid: number[][]) {
  ...
  return voronoi.cells
    .map((cell: Cell, i: number) => drawElement(cell, style(i, config.color, config.style)))
    .join('');
}

function boundingBox(grid: number[][]): BoundingBox {
  const { min, max } = grid.reduce((agg, pos, i) => {
...
function drawElement(cell: Cell, style: string) {
  return `<path ${style} d="${cell.path.reduce((s: string, point: Point, i: number) => {
    if (i === 0) {
