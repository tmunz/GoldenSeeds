...
export class ConvexPolygonTools {
  static offsetPath(path: Point[], offset = 0): Point[] {
    ...
    const counterclockwisePath = isCounterclockwise
      ? path
      : [...path].reverse();

    const rawOffsets = ConvexPolygonTools.calculateRawOffsets(
    ...
    rawOffsets.forEach((curr, i, arr) => {
      ...
      const originalDistance = Math2d.distanceBetween(
        prev.originalPoint,
        curr.originalPoint,
      );
      const currOverlapsWithPrev =
        originalDistance + offset < curr.baseDistance + prev.baseDistance;
      let cleanedPoint = curr.offsetPoint;

      if (currOverlapsWithPrev) {
        cleanedPath.pop();
        const sideA = Math2d.distanceBetween(
          curr.offsetPoint,
          prev.offsetPoint,
        );
        ...
        const newPointDistanceFromPrevOffsetPoint = Math2d.triangleSideB(
          sideA,
          alpha,
          beta,
        );
  ...
  private static calculateRawOffsets(
    ...
    return path.map((point, i, arr) => {
      ...
      const baseDistance = Math2d.triangleSideB(
        offset,
        bisectorAngle,
        Math.PI / 2,
      );
      const offsetPoint: Point = Math2d.calculatePointWithAngleAndDistance(
        point,
        next,
        bisectorAngle,
        baseDistance,
