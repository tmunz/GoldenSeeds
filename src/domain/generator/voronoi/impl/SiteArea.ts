...
export class SiteArea {
  ...
  close(createBorderEdge: (site: Site, vertexA: Point, vertexB: Point) => Edge, boundary: Boundary) {
    ...
    if (halfEdges.length > 0 && this.canBeClosed) {
      const danglingStart: HalfEdge = halfEdges.find((halfEdge) => boundary.isPointOnBorder(halfEdge.getStartpoint()));
      const danglingEnd: HalfEdge = halfEdges.find((halfEdge) => boundary.isPointOnBorder(halfEdge.getEndpoint()));
