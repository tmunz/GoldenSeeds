...
export class ColorInput extends React.Component<Props> {
  ...
  private convertValue(channel: any, inputColor?: InputColor) {
    const value = this.props.rangeValue;
    return channel < 0 ? -0x80000000 | value
      : 0xff < channel ? 0x1000000 | value
        : inputColor === InputColor.RED ? (value & 0x00ffff | ((Number(channel) & 0xff) << 16))
          : inputColor === InputColor.GREEN ? (value & 0xff00ff | ((Number(channel) & 0xff) << 8))
            : inputColor === InputColor.BLUE ? (value & 0xffff00 | ((Number(channel) & 0xff) << 0))
              : undefined;
  }
}
